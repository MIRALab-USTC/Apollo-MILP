...
def collect(ins_dir,q,sol_dir,log_dir,bg_dir,settings):
    ...
    while True:
        ...
        A2,v_map2,v_nodes2,c_nodes2,b_vars2=get_a_gp_fast(filepath)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nWorkers', type=int, default=24)
    ...
    for size in sizes:
        ...
        if not os.path.isdir(f'./dataset/{size}_train'):
            os.mkdir(f'./dataset/{size}_train')
        if not os.path.isdir(f'./dataset/{size}_train/solution'):
            os.mkdir(f'./dataset/{size}_train/solution')
        if not os.path.isdir(f'./dataset/{size}_train/NBP'):
            os.mkdir(f'./dataset/{size}_train/NBP')
        if not os.path.isdir(f'./dataset/{size}_train/logs'):
            os.mkdir(f'./dataset/{size}_train/logs')
        if not os.path.isdir(f'./dataset/{size}_train/BG'):
            os.mkdir(f'./dataset/{size}_train/BG')

        SOL_DIR =f'./dataset/{size}_train/solution'
        LOG_DIR =f'./dataset/{size}_train/logs'
        BG_DIR =f'./dataset/{size}_train/BG'
